...
class Crawler(threading.Thread):
    def __init__(self, base_url, links_to_crawl, have_visited, error_links, url_lock):
        ...
        self.have_visited = have_visited # visited urls
        self.error_links = error_links # urls throwing errors
            ...
            try:
                ...
                for a_tag in soup.find_all('a'):
                    ...
                    if (self.base_url[len(self.base_url)-1] == "/"):
                        b_url = self.base_url[8:len(self.base_url) -1]
                    else:
                        b_url = self.base_url[8:]
                    if (a_tag.get("href") not in self.have_visited) and (urlparse(link).netloc == b_url):
